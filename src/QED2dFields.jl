...
function unfold_reflect!(Uin, prm::LattParm, kprm::KernelParm)

    Uout = CUDA.ones(ComplexF64, 2*prm.iL[1], 2*prm.iL[2], 2)    
    CUDA.@sync begin
        CUDA.@cuda threads=kprm.threads blocks=kprm.blocks krnl_reflect!(Uout, Uin, prm.iL[1], prm.iL[2])
    end

    return Uout
end

function krnl_reflect!(Uout, Uin, l1old, l2old)
    ...
    for id in 1:2
        Uout[i1,i2,id]             = Uin[i1,i2,id]
        Uout[i1+l1old,i2,id]       = conj(Uin[i1,i2,id])
        Uout[i1,i2+l2old,id]       = conj(Uin[i1,i2,id])
        Uout[i1+l1old,i2+l2old,id] = Uin[i1,i2,id]
    end
